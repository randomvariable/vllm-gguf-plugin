...
native CUDA/HIP GEMV plus its own gated Triton GEMM. Outside that gate every
ROCmFPX format falls back to dequantize-plus-dense. Native MoE support is not
claimed, and only type 101 has any native GEMV. These paths therefore run on consumer
GPUs without FP8/FP4 tensor cores:
