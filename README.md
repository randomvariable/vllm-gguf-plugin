...
| **Limited** | Ternary and low-bit extended types: `TQ1_0`, `TQ2_0`, `Q2_0`, type-41 `Q1_0` / `Q1_0_G128` | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | Native CUDA/HIP dequantization plus Triton GEMM and Triton MoE. No native GEMV/GEMM/MoE kernel exists, so small-batch work dequantizes and falls back to dense. Type 41 is accepted with geometry `(128, 18)`; incompatible type-41 geometry is rejected. Validated on Radeon 8060S (`gfx1151`, ROCm/HIP 7.14.60850, Torch `2.12.0+rocm7.14.0`, Triton 3.7.1, `TRITON_ALLOW_NON_CONSTEXPR_GLOBALS=1`) against independent per-format ABI oracles transcribed from llama.cpp `ggml-quants.c` (sequential LSB-first for `Q1_0`/`Q2_0`, plane-major for `TQ2_0`, base-3 packing for `TQ1_0`), covering FP32/FP16/BF16, rank-2/rank-3 activations, tail output rows, and multi-block rows. This is kernel and dispatch validation, not full vLLM end-to-end/model-load validation. |
