"""Dedicated dense GEMM for GGML type 102 (Q6_0_ROCMFPX).

Type 102 packs 32 6-bit sign-magnitude codes into 24 bytes followed by two
UE4M3 half-scales (bytes 24 and 25), giving a 26-byte block. Code ``j``
occupies bits ``[6j, 6j+6)`` of the payload, so codes may straddle a byte
boundary. Decoding is sign-magnitude: ``magnitude = code & 31`` and a set
sign bit (``code & 32``) negates it, with the special case that magnitude
zero plus the sign bit decodes to ``-32`` rather than ``-0``. The first
scale covers outputs 0..15, the second covers 16..31. Reserved scale bytes
``0x7F..0xFF`` decode to zero.
"""
...
# hidden=4096, M=1. Widening BLOCK_N to 32 measured 442us (82.9 GB/s) versus
# 613us (59.8 GB/s) with BLOCK_N=16.
LAUNCH_BLOCK_M = 16
LAUNCH_BLOCK_N = 32
LAUNCH_NUM_WARPS = 1
LAUNCH_NUM_STAGES = 2
...
def _gemm_kernel(
    ...
    for block in range(num_blocks):
        ...
        w_tile = signed * _decode_scale(scale_byte)
...
def _decode_cpu(weights: torch.Tensor) -> torch.Tensor:
    ...
    codes = ((low >> bit_shift) | (high << (8 - bit_shift))) & 0x3F

    magnitude = codes & 31
    signed = torch.where(
        (codes & 32) != 0,
        -torch.where(magnitude != 0, magnitude, torch.full_like(magnitude, 32)),
        magnitude,
    ).to(torch.float32)
    ...
    return (signed * scales).reshape(weights.shape[0], -1)


def ggml_gemm_q6_0_rocmfpx_triton(
    W: torch.Tensor, X: torch.Tensor, row: int
) -> torch.Tensor:
    if W.dim() != 2:
        raise ValueError(f"Q6_0_ROCMFPX weights must be 2D, got {W.dim()}D")
    if W.dtype is not torch.uint8:
        raise TypeError(f"Q6_0_ROCMFPX weights must be uint8, got {W.dtype}")
    if X.dtype not in SUPPORTED_ACTIVATION_DTYPES:
        raise TypeError(
            "Q6_0_ROCMFPX supports float16, bfloat16, and float32 activations, "
            f"got {X.dtype}"
        )
    if X.dim() not in (2, 3):
        raise ValueError(f"X must be 2D or 3D, got {X.dim()}D")
    if any(size <= 0 for size in X.shape):
        raise ValueError("Q6_0_ROCMFPX activations must have positive dimensions")
    if W.shape[0] <= 0 or W.shape[1] <= 0:
        raise ValueError("Q6_0_ROCMFPX weights must have positive dimensions")
    if row != W.shape[0]:
        raise ValueError(
            f"row must match W.shape[0], got row={row}, W.shape[0]={W.shape[0]}"
        )
    if W.shape[1] % BLOCK_BYTES:
        raise ValueError(f"Q6_0_ROCMFPX row width must be divisible by {BLOCK_BYTES}")
    ...
    if W.device != X.device:
        raise ValueError(
            "Q6_0_ROCMFPX weights and activations must be on the same device"
        )
