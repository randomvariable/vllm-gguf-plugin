"""Dedicated dense GEMM for GGML type 42 (Q2_0).
...
``j//4``; four codes fit exactly per byte so none straddle a boundary. The
codes are offset binary: ``00 -> -1, 01 -> 0, 10 -> +1, 11 -> +2``, decoded as
``(code - 1) * d``.
"""
...
def ggml_gemm_q2_0_triton(
    W: torch.Tensor,
    X: torch.Tensor,
    row: int,
) -> torch.Tensor:
    """Dense GEMM for Q2_0 packed weights."""
    if W.dim() != 2:
        raise ValueError(f"Q2_0 weights must be 2D, got {W.dim()}D")
    if W.dtype is not torch.uint8:
        raise TypeError(f"Q2_0 weights must be torch.uint8, got {W.dtype}")
    if X.dtype not in SUPPORTED_ACTIVATION_DTYPES:
        raise TypeError(
            "Triton Q2_0 kernel supports torch.float16, torch.bfloat16, and "
    ...
    if W.shape[1] % BLOCK_BYTES != 0:
        raise ValueError(
            f"Invalid Q2_0 row width {W.shape[1]}: must be divisible by {BLOCK_BYTES}"
        )
    if not W.is_cuda or not X.is_cuda:
        raise ValueError("Q2_0 Triton kernel requires CUDA tensors")
    ...
    if X.shape[-1] != hidden_size:
        raise ValueError(
            ...
            f"Q2_0 weight width {hidden_size}"
        )
